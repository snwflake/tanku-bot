...
class WoTAPI():
    def __init__(self, app_id: str, clan_id: int) -> None:
        ...
        self.headers = {
            "User-Agent": "tankU-clan/python-requests"
        }
        self.appID = f"?application_id={app_id}"
        self.clanID = clan_id

    def _call(self, method: str, endpoint: str, params: dict | None = {}, data: dict | None = {}) -> dict | HTTPException:
        print(f'{datetime.datetime.now()}: {method} {endpoint} called')
        resp = request(method, self.baseURL + endpoint + self.appID, params=params, data=data, headers=self.headers)
